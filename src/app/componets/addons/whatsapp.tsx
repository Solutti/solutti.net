...
export function WhatsApp(){
    ...
    return(
        <div className={whatsapp == true ? "fixed" : " hidden"}> {/* Altere esta configuração no arquivo config.tsx use "true" para exibir na página ou "false" para remover*/}
            <div className={animado == true ? "z-30 fixed bottom-5 right-5 w-20 h-20 flex items-center justify-center duration-200 transition-all animate-bounce hover:animate-none" : "z-30 fixed bottom-5 right-5 w-20 h-20 flex items-center justify-center"}>
                ...
                <a href={`https://api.whatsapp.com/send?phone=${numero}`} target="_blank" aria-label="Entre em contato via WhatsApp">
                    <Image src={Logo} alt="Logotipo WhatsApp" className=" max-w-[80px] bg-contain bg-center"/>
                </a>
            </div>
        </div>
    )
